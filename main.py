...
def generate_click_track(song, downbeat_file, non_downbeat_file, output_file):
    ...
    for signature in song['time_signatures']:
        ...
        # get the offset, default to 0 if not provided
        ...
        for measure in range(signature['measures']):
            for beat in range(signature['numerator']):
                ...
                elif "sub_beat_multiplier" in signature and beat % signature["sub_beat_multiplier"] == 0:
                    final_track += subbeat[:beat_duration_ms]
                else:
...
def overlay_click_track(song_file, click_track, output_file, global_offset_ms):
    ...
    if global_offset_ms > 0:
        click_track = AudioSegment.silent(
            duration=global_offset_ms) + click_track
    ...
    if len(click_track) < len(song):
        click_track += AudioSegment.silent(
            duration=(len(song) - len(click_track)))
...
def create_video_with_text(song, output_audio, output_video, album_art_path, total_duration, global_offset_ms):
    ...
    for signature in song['time_signatures']:
        ...
        for _ in range(1, measures + 1):
            ...
            txt_clip = TextClip(txt, fontsize=24, color='white').set_duration(
                measure_duration)
            txt_clip = txt_clip.set_position(
                ('center', time_signature_y)).set_start(current_time)
    ...
    # Load album art
    album_art = ImageClip(album_art_path).set_start(0).set_duration(total_duration).resize(
        height=360).set_position(('center', 'center'))  # Adjust size as needed

    # Title and album text
    title_clip = TextClip(song["title"], fontsize=40, color='white',
                          bg_color='black').set_start(0).set_duration(total_duration)
    album_clip = TextClip(song["album"], fontsize=30, color='white',
                          bg_color='black').set_start(0).set_duration(total_duration)
    artist_clip = TextClip(song["artist"], fontsize=30, color='white',
                           bg_color='black').set_start(0).set_duration(total_duration)

    title_clip = title_clip.set_position(('center', 'top')).margin(top=20)
    album_clip = album_clip.set_position(('center', title_clip.size[1]))
    artist_clip = artist_clip.set_position(
        ('center', album_clip.size[1] + title_clip.size[1]))
    ...
    video = CompositeVideoClip(clips, size=(
        1280, 720)).set_duration(total_duration)
...
input_file_path = 'songs/Messiah Complex II.jsonc'
downbeat_file_path = 'down.wav'
non_downbeat_file_path = 'up.wav'
subbeat_file_path = "subbeat.wav"
song_file_path = r"C:\Users\Tyson\Music\iTunes\iTunes Media\Music\Haken\Virus\1-07 Messiah Complex II_ A Glutton F.m4a"
...
click_track = generate_click_track(
    song, downbeat_file_path, non_downbeat_file_path, output_file_path)
total_length = overlay_click_track(
    song_file_path, click_track, output_file_path, song["initial_offset_ms"])
create_video_with_text(song, output_file_path, output_video_path,
                       album_art_path, total_length, song["initial_offset_ms"])
